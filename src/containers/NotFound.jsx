import React from 'react';
import '../assets/styles/components/NotFound.scss';

const NotFound = () => (
  // <React.Fragment></React.Fragment>
  // Esos son los mismos fragment
  // Los fragment sirven para enviar componentes al dom sin utilziar un div
  <>
    <div className="body_notfound">
      <div className="noise" />
      <div className="overlay" />
      <div className="terminal">
        <h1>
          Error
          <span className="errorcode">404</span>
        </h1>
        <p className="output">The page you are looking for might have been removed, had its name changed or is temporarily unavailable.</p>
        <p className="output">
          Please try to
          <a className="a" href="#1"> go back </a>
          or
          <a className="a" href="#2"> return to the homepage </a>
          .
        </p>
        <p className="output"> Good luck. </p>
      </div>
    </div>
  </>
);

export default NotFound;
